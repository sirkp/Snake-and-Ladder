...
void drawbox(int right,int left,int top,int bottom,int ileft,int itop,int horizontal,int vertical,int xcentre[],int ycentre[])
{
   for(int i=0;i<9;i++)                     
   {
        ycentre[i]=itop+(vertical/2);
...
void makeladder(int p1,int p2)
{
    ...
    double distance=(double)sqrt(((double)((v[p1].first-v[p2].first)*(v[p1].first-v[p2].first))+(double)((v[p1].second-v[p2].second)*(v[p1].second-v[p2].second))));
    int no=distance/18;
    //no--;
    setcolor(LIGHTBLUE);
    setlinestyle(0,0,3);
